...
export const payloadSiteEngine: SiteEngine = {
  ...
  async getCatalog(query: CatalogQueryDto = {}): Promise<CatalogPageDto> {
    ...
    const result = await getPublicCatalog({
      ...
      ...query.buildingType ? { buildingType: query.buildingType } : {},
      ...query.commercialType ? { buildingState: query.commercialType } : {},
      ...query.landUseType ? { buildingState: query.landUseType } : {},
      ...query.renovation ? { buildingState: query.renovation } : {},
  ...
  async getOffices() {
    return [{
      id: `${siteIdentity.city.slug}-office`,
      title: `Офис ${siteIdentity.brand} в ${siteIdentity.city.prepositional}`,
      address: siteIdentity.contacts.address ?? '',
      mapUrl: siteIdentity.contacts.address ? buildYandexMapWidgetURL(siteIdentity.contacts.address) : '',
      photoUrl: null,
    }]
  },
