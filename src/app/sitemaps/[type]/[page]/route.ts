...
export async function GET(_request: Request, context: { params: Promise<{ page: string; type: string }> }) {
  ...
  if (!sitemapTypes.has(typeValue as SitemapType) || !Number.isInteger(page) || page < 0) {
    return new Response(null, { status: 404 })
  }
  const documents = await getSitemapChunk(typeValue as SitemapType, page, sitemapPathFor)
