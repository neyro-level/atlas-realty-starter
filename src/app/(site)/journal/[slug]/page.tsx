...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const slug = (await params).slug; const article = await (await getSiteEngine()).getArticle(slug); if (!article) return { title: "Материал не найден", robots: { index: false, follow: false } }; return buildSeoMetadata({ path: `/journal/${slug}`, title: (article.seoTitle ?? article.title).replace(/\s*\|\s*«АТЛАС»\s*$/u, "").trim(), description: article.seoDescription ?? getArticleExcerpt(article), image: article.coverImage }); }
