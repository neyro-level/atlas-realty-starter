...
export const metadata = buildSeoMetadata({ path: "/sitemap", title: "Карта сайта", description: "HTML-карта публичных страниц сайта «АТЛАС»: недвижимость, новостройки, журнал, компания, правовые документы и объекты." });
...
export default async function HtmlSitemapPage() {
  ...
  const services: SitemapColumnDto[] = [
    ...
    { title: "Компания", items: [{ href: "/", label: "Главная" }, ...itemsFromCorporate(["kontakty", "o-kompanii", "rabota-rieltorom", "reviews"]), { href: "/sotrudniki", label: "Сотрудники" }, { href: "/sitemap", label: "Карта сайта" }] },
    { title: "Личные подборки", items: [{ href: "/izbrannoe", label: "Избранное" }, { href: "/sravnenie", label: "Сравнение" }] },
  ];
  ...
  const articleColumns = chunkItems(articles.map((article) => ({ href: `/journal/${article.slug}`, label: article.title })), 4);
  const journal: SitemapColumnDto[] = [{ title: "Рубрики", items: [{ href: "/journal", label: "Журнал агентства", emphasis: true }, ...journalCategories.map((category) => ({ href: `/journal/category/${category.slug}`, label: category.title, emphasis: true }))] }, ...articleColumns.map((items, index) => ({ title: index === 0 ? "Статьи" : undefined, items }))];
  const legal: SitemapColumnDto[] = [{ title: "Документы", items: [{ href: "/legal", label: "Правовая информация" }, ...legalDocuments.map((document) => ({ href: `/${document.slug}`, label: document.shortTitle }))] }];
  ...
  return <HtmlSitemapView page={page} breadcrumbs={<Breadcrumbs items={[{ label: "Главная", href: "/" }, { label: "Карта сайта" }]} className="mb-5" />} linkRenderer={SitemapLink} />;
}

function itemsFromCatalog(slugs:string[]):SitemapLinkDto[]{return slugs.flatMap((slug)=>{const item=catalogPresets.find((entry)=>entry.slug===slug);return !item||item.indexing!=="index"?[]:[{href:`/${item.slug}`,label:item.navLabel}];});}
...
function getCorporateItem(slug:string):SitemapLinkDto|null{const page=getCorporatePage(slug);return page?{href:`/${page.slug}`,label:page.navLabel}:null;}
function filterExisting(items:SitemapLinkDto[],paths:Set<string>){return items.filter((item)=>paths.has(item.href));}
function pageItems(kind:"objects"|"reserve",total:number,pageSize=200):SitemapLinkDto[]{const pages=Math.ceil(total/pageSize);return Array.from({length:pages},(_,index)=>({href:`/sitemap/${kind}/${index+1}`,label:`Страница ${index+1} из ${pages}`,countAsPublishedPage:false}));}
