...
export function NewBuildingDecisionSidebar({ complex }: { complex: NewBuilding }) {
  ...
  return (
    <NewBuildingDecisionSidebarView
      ...
      mortgageAction={<RequestCta label="Одобрить ипотеку" complexName={complex.name} slug={complex.slug} complexId={complex.sourceId} modalTitle={`Одобрить ипотеку в ${complex.name}`} showIcon={false} className="min-h-10 w-full bg-[var(--surface-dark)] text-xs text-white hover:bg-[var(--new-building-sidebar-surface-hover)]" />}
