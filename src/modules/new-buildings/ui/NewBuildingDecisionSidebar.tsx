...
export function NewBuildingDecisionSidebar({ complex }: { complex: NewBuilding }) {
  ...
  return (
    <NewBuildingDecisionSidebarView
      ...
      mortgageAction={<RequestCta label="Одобрить ипотеку" complexName={complex.name} slug={complex.slug} complexId={complex.sourceId} modalTitle={`Одобрить ипотеку в ${complex.name}`} showIcon={false} className="min-h-10 w-full bg-[var(--surface-dark)] text-label text-white hover:bg-[var(--new-building-sidebar-surface-hover)]" />}
      availabilityAction={<RequestCta label="Узнать наличие квартир" complexName={complex.name} slug={complex.slug} complexId={complex.sourceId} variant="secondary" modalTitle={`Узнать наличие квартир в ${complex.name}`} showIcon={false} className="min-h-10 w-full border-[var(--border)] bg-[var(--background)] text-label text-[var(--text-primary)] hover:border-[var(--accent)] hover:bg-[var(--accent-soft)] hover:text-[var(--accent)]" />}
    />
  );
}
