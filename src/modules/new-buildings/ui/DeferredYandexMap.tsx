...
export function DeferredYandexMap({ widgetUrl, yandexUrl, title }: DeferredYandexMapProps) {
  ...
  return (
    <div className="relative h-full min-h-[260px] w-full overflow-hidden bg-[var(--surface-muted)] md:min-h-[300px] lg:min-h-[380px]" data-map-deferred={!active}>
      {active ? (
        <iframe
          src={widgetUrl}
          title={title}
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="absolute bottom-3 right-3 hidden min-h-10 items-center rounded-lg bg-white px-3 text-xs font-semibold text-[var(--text-primary)] shadow-[var(--deferred-yandex-map-shadow-01)] transition hover:text-[var(--accent)] lg:inline-flex"
      >
