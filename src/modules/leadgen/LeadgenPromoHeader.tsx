  ...
  return (
    <header
      ...
      className={`fixed z-[90] flex h-[68px] items-center border transition-all duration-300 lg:h-[78px] ${
        docked
          ? "left-3 right-3 top-3 rounded-[14px] border-[var(--leadgen-promo-header-border-primary)] bg-white shadow-[var(--leadgen-promo-header-shadow-primary)] lg:left-5 lg:right-5 lg:top-4"
    ...
    >
      <div className={`mx-auto flex h-full w-full max-w-[1160px] items-center gap-4 px-5 transition-all duration-300 sm:px-8 ${docked ? "lg:px-6" : ""}`}>
        ...
        <div className="hidden min-h-9 items-center justify-center gap-1.5 rounded-[9px] bg-[var(--leadgen-promo-header-surface-primary)] px-3.5 text-center text-[var(--leadgen-promo-header-content-secondary)] min-[1120px]:inline-flex">
          ...
          <span className="text-[13px] font-normal leading-tight text-[var(--leadgen-promo-header-content-tertiary)]">{content.headerTrust.label}</span>
        </div>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-4 lg:gap-6">
          {content.phone && content.phoneHref ? (
            ...
            >
              <span className="grid leading-tight">
                ...
                {content.hours ? <span className="text-[10px] font-medium text-[var(--leadgen-promo-header-content-subtle)]">{content.hours}</span> : null}
              </span>
            </a>
          ) : null}

          <LeadgenRequestButton
            className="hidden min-h-11 items-center rounded-[10px] bg-[var(--accent)] px-5 text-sm font-bold text-white shadow-[var(--leadgen-promo-header-shadow-tertiary)] transition hover:-translate-y-0.5 hover:bg-[var(--accent-hover)] hover:shadow-[var(--leadgen-promo-header-shadow-subtle)] sm:inline-flex"
            mode="request"
          ...
          <Button variant="plain"
            ...
            className="inline-flex items-center justify-center rounded-[10px] border border-[var(--leadgen-promo-header-border-primary)] bg-white text-[var(--text-primary)] transition hover:border-[var(--accent)] hover:text-[var(--accent)] lg:hidden"
      ...
      <div
        ...
        className={`absolute left-3 right-3 top-[calc(100%+10px)] rounded-[14px] border border-[var(--leadgen-promo-header-border-primary)] bg-white p-4 shadow-[var(--leadgen-promo-header-shadow-muted)] transition lg:hidden ${
          menuOpen ? "visible translate-y-0 opacity-100" : "invisible -translate-y-2 opacity-0 pointer-events-none"
        }`}
      >
        <div className="flex items-center gap-3">
          <div className="relative size-12 shrink-0 overflow-hidden rounded-[8px] bg-[var(--leadgen-promo-header-surface-secondary)]">
            <Image
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[11px] font-medium leading-4 text-[var(--accent)]">{content.manager.role}</p>
          </div>
        </div>

        <p className="mt-3.5 text-[12px] font-normal leading-[1.58] text-[var(--leadgen-promo-header-content-primary)]">
          {content.mobileMenuText ?? mobileMenuText}
        </p>

        <div className="mt-3.5 grid gap-2">
          <LeadgenRequestButton
            className="inline-flex min-h-12 items-center justify-center rounded-[10px] bg-[var(--accent)] px-5 text-[15px] font-semibold text-white transition hover:bg-[var(--accent-hover)]"
            mode="quiz"
          ...
          {content.phone && content.phoneHref ? (
            <a
              ...
              data-analytics-item={content.route}
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[10px] border border-[var(--border)] bg-white px-5 text-[15px] font-semibold text-[var(--text-primary)] transition hover:border-[var(--leadgen-promo-header-border-secondary)] hover:bg-[var(--leadgen-promo-header-surface-tertiary)] hover:text-[var(--accent)]"
            >
