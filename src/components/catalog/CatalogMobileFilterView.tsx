          ...
          <SheetContent side="bottom" className="inset-x-0 bottom-0 max-h-[var(--viewport-dialog-max-height)] overflow-y-auto rounded-t-2xl border-0 bg-[var(--surface-card)] p-4 pb-[var(--spacing-safe-footer)] shadow-[var(--catalog-mobile-filter-shadow-drawer)] lg:hidden" showClose>
            ...
            <div className="grid gap-3">
              ...
              <label className="block text-caption font-semibold uppercase text-[var(--text-muted)]">
                Порядок
                <Select variant="native" value={draft.sort} onChange={(event) => setDraft((current) => ({ ...current, sort: event.target.value }))} className="mt-1.5 min-h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3 text-support font-medium normal-case text-[var(--text-primary)]">
