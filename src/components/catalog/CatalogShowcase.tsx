...
type Props = {
  ...
  result: CatalogPageData['result']
}

export function CatalogShowcase({ basePath, complexes = [], content, filters, heroImage, preset, properties = [], result }: Props) {
  const complexMode = preset.mode === 'new_building'
  const count = result.totalDocs

  return (
    <main>
      <section className="bg-white py-5 lg:py-7"><div className="mx-auto max-w-site-frame px-5 lg:px-10"><Breadcrumbs items={[{ href: content.breadcrumbs.homeHref, label: content.breadcrumbs.homeLabel }, { href: content.breadcrumbs.allRealtyHref, label: content.breadcrumbs.allRealtyLabel }, { label: preset.title }]} /></div></section>
        ...
        <div className="mx-auto max-w-site-frame px-5 lg:px-10">
          <div className="relative min-h-[310px] overflow-hidden rounded-lg bg-[#17161a] text-white lg:min-h-[390px]">
            ...
            <div className="relative flex min-h-[310px] max-w-3xl flex-col justify-end p-7 lg:min-h-[390px] lg:p-12">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-white/60">{content.eyebrow}</p>
      ...
      <section className="catalog-section">
        <div className="mx-auto max-w-site-frame px-5 lg:px-10">
          <div className="catalog-tabs mb-5">
            {content.tabs.map((tab) => <Link className={`catalog-tab ${basePath === tab.href ? 'is-active' : ''}`} href={tab.href} key={tab.href}><span className="catalog-tab__label">{tab.label}</span></Link>)}
          </div>
          ...
          ) : <CatalogEmptyState complexMode={complexMode} content={content} />}
...
function CatalogEmptyState({ complexMode, content }: { complexMode: boolean; content: CatalogPageContent }) {
  return <div className="mt-7 rounded-lg border border-[#e3e3e1] bg-white px-6 py-14 text-center shadow-sm"><h2 className="text-2xl font-bold">{complexMode ? content.empty.complexTitle : content.empty.propertyTitle}</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[#827f81]">{content.empty.description}</p><Link className="mt-6 inline-flex min-h-11 items-center rounded-lg bg-[#8a1515] px-5 font-semibold text-white" href={content.empty.selectionHref}>{content.empty.selectionLabel}</Link></div>
}
