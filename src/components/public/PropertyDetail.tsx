...
export function PropertyDetail({ property, related }: { property: PublicProperty; related: PublicProperty[] }) {
  ...
  return (
    <main className="min-h-screen overflow-clip bg-[#FAFAFA] text-[#17161A]">
      <PropertyMobileTopBar property={property} />
      <div className="mx-auto max-w-site-frame px-2.5 pb-14 pt-3 sm:px-3 lg:px-10 lg:pb-20 lg:pt-10 xl:px-12">
        <Breadcrumbs className="mb-5 max-lg:hidden" items={[{ href: '/', label: 'Главная' }, { href: '/nedvizhimost-rostov', label: 'Недвижимость' }, { href: '/kvartiry-rostova', label: 'Квартиры' }, { label: property.title }]} />
        <section className="grid w-full items-start gap-6 lg:grid-cols-[minmax(0,calc(100%-324px))_300px]">
          <div className="grid min-w-0 gap-4">
            <MediaGallery address={property.address || 'Ростов-на-Дону'} images={property.images} latitude={property.latitude} longitude={property.longitude} name={property.title} videoUrl={property.videoUrl} />
            ...
            <PropertyDecisionSidebar className="lg:hidden" property={property} />
            ...
            <ViewingRequestSection property={property} />
          ...
          <PropertyDecisionSidebar className="hidden lg:block" property={property} />
        </section>
      </div>
    </main>
  )
...
function buildSummary(property: PublicProperty): Summary[] { return [{ icon: property.isStudio ? Building : BedDouble, label: 'Комнаты', value: property.isStudio ? 'Студия' : property.rooms ? `${property.rooms}-комнатная` : 'Уточняется' }, { icon: Ruler, label: 'Общая площадь', value: property.totalArea ? `${number(property.totalArea)} м²` : 'Уточняется' }, { icon: Building2, label: 'Этаж', value: property.floor ? `${property.floor}${property.floorsTotal ? ` из ${property.floorsTotal}` : ''}` : 'Уточняется' }, { icon: MapPin, label: 'Район', value: property.district || 'Ростов-на-Дону' }] }
