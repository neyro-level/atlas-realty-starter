...
import { HomeArticlesPreviewView } from "@starter/site-ui/views";
...
export function HomeArticlesPreview({ articles }: { articles: ArticleSummary[] }) {
  ...
  const categories = journalCategories.map((category) => ({ slug: category.slug, title: category.title, href: routes.journalCategory(category.slug) }));
