...
export function SiteFooter({ contacts }: { contacts: PublicSiteContacts }) {
  ...
  ].filter((item): item is { label: string; href: string; icon: React.JSX.Element } => Boolean(item.href));
            ...
            <div className="site-footer__legal">
              {footer.legalLinks.map((link) =>
                link.external ? (
                  ...
                  <Link key={link.label} href={link.href} rel={link.nofollow ? "nofollow" : undefined}>
                    {link.label}
                  </Link>
                ),
              )}
