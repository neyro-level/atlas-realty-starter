...
export function Breadcrumbs({ items, className = "", tone = "light" }: BreadcrumbsProps) {
  ...
  const tones = {
    ...
    dark: {
      nav: "text-white/58",
      link: "shrink-0 whitespace-nowrap font-medium leading-5 transition hover:text-white",
      current: "min-w-0 truncate font-semibold leading-5 text-white",
      parent: "shrink-0 whitespace-nowrap font-medium leading-5 text-white/58",
      separator: "size-3.5 shrink-0 self-center text-white/30",
    },
    admin: {
      nav: "text-slate-500",
      link: "shrink-0 whitespace-nowrap font-medium leading-5 transition hover:text-sky-700",
      current: "min-w-0 max-w-[min(100%,28rem)] truncate font-semibold leading-5 text-slate-700",
      parent: "shrink-0 whitespace-nowrap font-medium leading-5 text-slate-500",
  ...
  const navClassName = [
    "breadcrumbs",
    "flex flex-nowrap items-center gap-x-2 overflow-x-auto py-0.5 text-sm leading-5",
    palette.nav,
    className,
  ]
    .filter(Boolean)
