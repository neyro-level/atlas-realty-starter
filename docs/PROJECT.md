...
- The UI Constitution migration is tracked by decreasing guard baselines. The semantic foundation is active and the residential-complex module no longer uses numbered component tokens; remaining compatibility aliases are retired domain by domain without changing Payload data or public DTO contracts.
