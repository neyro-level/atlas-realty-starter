...
Release input is a clean exact merged SourceCraft `main` SHA. The Pull Request pipeline installs the frozen lockfile, regenerates Payload artifacts, rejects generated-file drift and runs the daily `pnpm verify` contract. After merge, the Linux main pipeline regenerates and checks the Payload artifacts once more, builds Next.js and packages `.next/standalone` plus exact production dependencies. It does not repeat the unchanged PR verification suite. SourceCraft stores the archive as bounded parts; the manifest records their order, sizes, per-part SHA-256 values and the reconstructed archive SHA-256. The server never runs `pnpm install` or `pnpm build`.
