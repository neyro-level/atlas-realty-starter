...
if [[ ! "${APP_SLUG}" =~ ^[a-z][a-z0-9-]{2,48}$ || ! "${RELEASE_SHA}" =~ ^[0-9a-f]{40}$ || ! "${EXPECTED_SHA256}" =~ ^[0-9a-f]{64}$ ]]; then
  echo "Usage: install-release.sh <archive> <full-sha> <sha256> <app-slug>" >&2
  exit 1
fi

APP_USER="${APP_SLUG}"
APP_ROOT="/opt/${APP_SLUG}"
ENV_FILE="/etc/${APP_SLUG}/runtime.env"
...
previous_release=""
if [[ -L "${APP_ROOT}/current" ]]; then previous_release="$(readlink -f "${APP_ROOT}/current")"; fi
...
source "${ENV_FILE}"
set +a
export NODE_ENV=production RELEASE_SHA NEXT_PUBLIC_RELEASE_SHA="${RELEASE_SHA}" HOME="${APP_ROOT}"
export COREPACK_HOME="${APP_ROOT}/.cache/node/corepack" PNPM_HOME="${APP_ROOT}/.local/share/pnpm"
...
printf 'RELEASE_SHA=%s\n' "${RELEASE_SHA}" > "/etc/${APP_SLUG}/release.env"
chmod 0640 "/etc/${APP_SLUG}/release.env"
chown root:"${APP_USER}" "/etc/${APP_SLUG}/release.env"
...
if [[ -n "${previous_release}" && -d "${previous_release}" ]]; then
  ...
  printf 'RELEASE_SHA=%s\n' "${previous_sha}" > "/etc/${APP_SLUG}/release.env"
