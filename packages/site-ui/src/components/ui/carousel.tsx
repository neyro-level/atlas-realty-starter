...
function Carousel({ orientation = "horizontal", opts, setApi, plugins, className, children, ...props }: React.ComponentProps<"div"> & CarouselProps) {
  ...
  return (
    <CarouselContext.Provider value={{ carouselRef, api, opts, orientation, setApi, plugins, scrollPrev, scrollNext, canScrollPrev, canScrollNext }}>
...
function CarouselContent({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div ref={carouselRef} className="h-full overflow-hidden" data-slot="carousel-content">
      <div className={cn("flex h-full", orientation === "horizontal" ? "-ml-4" : "-mt-4 flex-col", className)} {...props} />
    </div>
  );
}
