...
export function BreadcrumbList({ className, ...props }: ComponentProps<"ol">) {
  return <ol data-slot="breadcrumb-list" className={cn("flex flex-wrap items-center gap-1.5 text-sm text-[#777375]", className)} {...props} />;
}
...
export function BreadcrumbSeparator({ className, children, ...props }: HTMLAttributes<HTMLLIElement>) {
  return (
    <li data-slot="breadcrumb-separator" aria-hidden="true" className={cn("text-[#AAA7A0]", className)} {...props}>
