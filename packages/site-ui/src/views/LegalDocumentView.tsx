...
export function LegalDocumentView({ document, legalName, email, linkRenderer: LinkRenderer }: LegalDocumentViewProps) {
  return (
    <main className="bg-[#F5F5F3] text-[#1C1B1F]">
      <header className="border-b border-[#DEDEDA] bg-white">
        <div className="mx-auto max-w-[1180px] px-5 py-8 md:px-8 md:py-10">
          ...
          <p className="mt-4 max-w-[760px] text-[15px] leading-6 text-[#666267] md:text-base md:leading-7">{document.description}</p>
          <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs font-semibold text-[#777378]">
      ...
      <div className="mx-auto grid max-w-[1180px] gap-6 px-5 py-8 md:px-8 md:py-12 lg:grid-cols-[250px_minmax(0,1fr)] lg:items-start">
        <Card className="rounded-lg border-[#DEDEDA] bg-white p-4 lg:sticky lg:top-[126px]" aria-label="Содержание документа">
          <p className="px-2 pb-3 text-xs font-extrabold uppercase text-[#807C80]">Содержание</p>
          <nav className="space-y-1">
            {document.sections.map((section, index) => <a key={section.title} href={`#section-${index + 1}`} className="block rounded-md px-2 py-2 text-sm leading-5 text-[#555156] transition hover:bg-[#F5F5F3] hover:text-[var(--accent)]">{section.title}</a>)}
          </nav>
        </Card>

        <Card className="overflow-hidden rounded-lg border-[#DEDEDA] bg-white px-5 md:px-8">
          {document.sections.map((section, index) => (
            <section id={`section-${index + 1}`} key={section.title} className="scroll-mt-32 pt-7 last:pb-7 md:pt-9 md:last:pb-9">
              <h2 className="text-xl font-extrabold leading-tight text-[#201F22] md:text-2xl">{section.title}</h2>
              {section.paragraphs?.length ? <div className="mt-4 space-y-3 text-[15px] leading-[1.65] text-[#555156] md:text-base">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div> : null}
              {section.items?.length ? <ul className="mt-4 space-y-2.5 text-[15px] leading-[1.6] text-[#555156] md:text-base">{section.items.map((item) => <li key={item} className="grid grid-cols-[8px_minmax(0,1fr)] gap-3"><span className="mt-[9px] size-1.5 rounded-full bg-[var(--accent)]" aria-hidden /><span>{item}</span></li>)}</ul> : null}
              {index < document.sections.length - 1 ? <Separator className="mt-7 bg-[#E6E6E2] md:mt-9" /> : null}
            </section>
          ))}
        </Card>
      </div>

      <section className="border-t border-[#DEDEDA] bg-white">
        <div className="mx-auto flex max-w-[1180px] flex-col gap-3 px-5 py-7 text-sm text-[#615D62] md:flex-row md:items-center md:justify-between md:px-8">
          <p>{legalName}</p>
