    ...
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-overline font-semibold uppercase tracking-spaced text-[var(--accent)]">Сообщение по объекту</p>
            <h2 id="property-chat-title" className="mt-3 max-w-100 text-heading-small font-semibold leading-card-compact text-[var(--text-primary)] sm:text-heading-compact">
        ...
        <form className="mt-7 grid gap-4 pb-2" onSubmit={onSubmit} data-analytics-form-type="property_chat" noValidate>
          ...
          {lockMessage ? (
            ...
            <label className="grid gap-2.5 text-support font-semibold text-[var(--text-primary)]">
              ...
              {showInlineHint ? <span className="text-label font-normal leading-step-body text-[var(--text-muted)]">Здесь можно написать свое сообщение.</span> : null}
              {errors.message ? <span className="text-label text-[var(--error)]">{errors.message}</span> : null}
            </label>
          )}
          ...
          <label className="grid gap-2.5 text-support font-semibold text-[var(--text-primary)]">
            ...
            <Input variant="plain"
              ...
              maxLength={18}
            />
            {errors.phone ? <span className="text-label text-[var(--error)]">{errors.phone}</span> : null}
          </label>
          ...
          {resultMessage ? <div className="rounded-md border border-[var(--property-chat-border-danger)] bg-[var(--property-chat-surface-danger)] px-3 py-2 text-body font-semibold text-[var(--error)]">{resultMessage}</div> : null}
