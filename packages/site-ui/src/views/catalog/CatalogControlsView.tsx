  ...
  return (
    <section id={sectionId} className={headline ? "bg-[var(--surface-card)] pb-10 pt-6 lg:pb-14 lg:pt-8" : "bg-[var(--surface-card)] pt-4 pb-10 md:pt-5 lg:py-14"} aria-label={heading || "Каталог недвижимости"}>
      <div className="mx-auto max-w-site-frame px-5">
        {headline ? <div className="mb-7 rounded-lg border border-[var(--catalog-controls-border-headline)] bg-[var(--surface-card-soft)] px-5 py-5 md:px-6 md:py-6"><h1 className="max-w-230 text-heading-small font-extrabold leading-[1.18] text-[var(--text-primary)] md:text-section-small lg:text-section-large">{headline}</h1></div> : null}
        {beforeControls}
...
export function CatalogEmptyStateView({ message, contactHref = "/kontakty", linkRenderer: Link }: { message: string; contactHref?: string; linkRenderer: SiteLinkRenderer }) {
  return <div className="mt-4 rounded-lg border border-dashed border-[var(--border)] bg-[var(--surface-card)] p-7 text-sm leading-6 text-[var(--text-secondary)]">{message}<div><Link href={contactHref} className="mt-4 inline-block font-bold text-[var(--accent)]">Попросить ручной подбор</Link></div></div>;
}
