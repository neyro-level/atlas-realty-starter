  ...
  return (
    <section className="bg-white py-10 lg:py-14" aria-labelledby="related-services-title">
      <div className="mx-auto max-w-site-frame px-5">
        <h2 id="related-services-title" className="text-2xl font-extrabold text-[var(--text-primary)]">Связанные юридические услуги</h2>
          ...
          {links.map((link) => (
            <Card key={link.href} className="rounded-lg border-[var(--border)] bg-[var(--surface-card-soft)] p-0 shadow-none transition hover:border-[var(--journal-border-hover)] hover:bg-[var(--accent-soft)]">
              <LinkRenderer href={link.href} className="group block p-5">
                <span className="block text-lg font-extrabold leading-tight text-[var(--text-primary)] transition group-hover:text-[var(--accent)]">{link.label}</span>
                <span className="mt-3 block text-sm leading-6 text-[var(--text-secondary)]">{link.description}</span>
              </LinkRenderer>
            </Card>
          ))}
        </div>
