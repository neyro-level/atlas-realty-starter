...
export function CatalogSearchFieldView({ name = "q", defaultValue, placeholder }: { name?: string; defaultValue?: string; placeholder: string }) {
  return <label className="flex min-h-12 items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-3"><Search className="size-5 text-[var(--text-muted)]" aria-hidden /><span className="sr-only">Поиск по каталогу</span><Input variant="plain" name={name} defaultValue={defaultValue ?? ""} placeholder={placeholder} className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--text-muted)]" /></label>;
}
...
export function CatalogSelectView({ name, value, options, title }: { name: string; value?: string; options: Array<{ value: string | number; label: string }>; title: string }) {
  return <label className="min-h-12 rounded-lg border border-[var(--border)] bg-white px-3 py-1 text-caption font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">{title}<Select variant="plain" name={name} defaultValue={value ?? ""} className="block w-full bg-transparent pt-0.5 text-sm font-semibold normal-case tracking-[0] text-[var(--text-primary)] outline-none"><option value="">Любой</option>{options.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></label>;
}
