  ...
  return (
    <section id={sectionId} className={headline ? "bg-white pb-10 pt-6 lg:pb-14 lg:pt-8" : "bg-white pt-4 pb-10 md:pt-5 lg:py-14"} aria-label={heading || "Каталог недвижимости"}>
      <div className="mx-auto max-w-site-frame px-5">
        {headline ? <div className="mb-7 rounded-lg border border-[var(--palette-e9e9e6)] bg-[var(--surface-card-soft)] px-5 py-5 md:px-6 md:py-6"><h1 className="max-w-[920px] text-[20px] font-extrabold leading-[1.18] text-[var(--text-primary)] md:text-[24px] lg:text-[26px]">{headline}</h1></div> : null}
        <div className="rounded-lg bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02),0_8px_24px_rgba(0,0,0,0.04)] lg:p-5">
...
export function CatalogLoadMoreView({ variant, loading, error, hasMore, onLoadMore, pages, linkRenderer: Link }: { variant: CatalogViewDto; loading: boolean; error: string | null; hasMore: boolean; onLoadMore: () => void; pages: CatalogPaginationItemDto[]; linkRenderer: SiteLinkRenderer }) {
  return <>
    {hasMore ? <div className={variant === "list" ? "flex justify-center py-8" : "col-span-full mt-4 flex justify-center"}><div className="flex flex-col items-center text-center"><button type="button" onClick={onLoadMore} disabled={loading} className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-[var(--surface-dark)] px-7 text-sm font-extrabold text-white shadow-[0_10px_22px_rgba(24,24,26,0.14)] transition hover:-translate-y-0.5 hover:bg-[var(--accent)] hover:shadow-[0_14px_30px_rgba(138,21,21,0.18)] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0 sm:min-w-[220px]">{loading ? <><Loader2 className="size-4 animate-spin" aria-hidden />Загружаем...</> : <>Показать ещё<ArrowDown className="size-4 transition group-hover:translate-y-0.5" aria-hidden /></>}</button>{error ? <p className="mt-3 text-sm font-semibold text-[var(--accent)]">{error}</p> : null}</div></div> : null}
    {pages.length ? <nav className="col-span-full mt-6 flex flex-wrap justify-center gap-2" aria-label="Страницы каталога">{pages.map((item) => item.href ? <Link key={item.key} href={item.href} ariaCurrent={item.current ? "page" : undefined} className={`grid min-h-11 place-items-center rounded-lg border px-3 text-sm font-bold ${item.current ? "border-[var(--surface-dark)] bg-[var(--surface-dark)] text-white" : "border-[var(--palette-d3d3cf)] bg-white text-[var(--surface-dark)]"}`}>{item.label}</Link> : <span key={item.key} className="grid min-h-11 min-w-11 place-items-center text-sm text-[var(--palette-6a676b)]">{item.label}</span>)}</nav> : null}
  </>;
}
...
export function CatalogSearchFieldView({ name = "q", defaultValue, placeholder }: { name?: string; defaultValue?: string; placeholder: string }) {
  return <label className="flex min-h-12 items-center gap-2 rounded-lg border border-[var(--border)] bg-white px-3"><Search className="size-5 text-[var(--text-muted)]" aria-hidden /><span className="sr-only">Поиск по каталогу</span><input name={name} defaultValue={defaultValue ?? ""} placeholder={placeholder} className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--text-muted)]" /></label>;
}
...
export function CatalogRangePairView({ from, to, label, fromValue, toValue }: { from: string; to: string; label: string; fromValue?: number; toValue?: number }) {
  return <div className="grid min-h-12 grid-cols-2 overflow-hidden rounded-lg border border-[var(--border)] bg-white"><label className="border-r border-[var(--border)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">{label} от<input name={from} type="number" min="0" defaultValue={fromValue ?? ""} className="block w-full bg-transparent pt-0.5 text-sm font-semibold normal-case tracking-[0] text-[var(--text-primary)] outline-none" /></label><label className="px-3 py-1 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">до<input name={to} type="number" min="0" defaultValue={toValue ?? ""} className="block w-full bg-transparent pt-0.5 text-sm font-semibold normal-case tracking-[0] text-[var(--text-primary)] outline-none" /></label></div>;
}
...
export function CatalogSelectView({ name, value, options, title }: { name: string; value?: string; options: Array<{ value: string | number; label: string }>; title: string }) {
  return <label className="min-h-12 rounded-lg border border-[var(--border)] bg-white px-3 py-1 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--text-muted)]">{title}<select name={name} defaultValue={value ?? ""} className="block w-full bg-transparent pt-0.5 text-sm font-semibold normal-case tracking-[0] text-[var(--text-primary)] outline-none"><option value="">Любой</option>{options.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>;
}
