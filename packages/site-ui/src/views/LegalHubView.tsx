...
export function LegalHubView({ documents, linkRenderer: LinkRenderer }: LegalHubViewProps) {
  return (
    <main className="min-h-[calc(100vh-106px)] bg-[var(--legal-hub-surface-primary)] px-4 py-8 text-[var(--legal-hub-content-primary)] sm:px-6 md:py-14 lg:py-16">
      <Card className="mx-auto max-w-[1140px] overflow-hidden rounded-lg border-[var(--legal-hub-border-primary)] bg-white px-5 py-7 shadow-[var(--legal-hub-shadow-primary)] sm:px-7 sm:py-9 md:px-10 md:py-11">
        ...
        <h1 className="text-[28px] font-semibold leading-tight text-[var(--legal-hub-content-tertiary)] md:text-[36px]">Правовая информация</h1>
        ...
        <Accordion type="multiple" className="mt-8 border-t border-[var(--legal-hub-border-secondary)] md:mt-10">
          {documents.map((document) => (
            <AccordionItem key={document.id} value={document.slug} className="group border-[var(--legal-hub-border-secondary)]">
              <AccordionTrigger
                className="min-h-[72px] py-5 text-[15px] font-medium leading-6 text-[var(--legal-hub-content-subtle)] hover:text-[var(--legal-hub-content-subtle)] md:min-h-[82px] md:py-6 md:text-base"
              ...
              <AccordionContent forceMount className="pb-7 pr-0 md:pb-8 md:pr-16">
                <p className="max-w-[760px] text-sm font-normal leading-6 text-[var(--legal-hub-content-strong)] md:text-[15px]">{document.description}</p>
                <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3">
                  <LinkRenderer href={`/${document.slug}`} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[var(--legal-hub-surface-tertiary)] px-4 text-sm font-semibold text-white transition hover:bg-[var(--legal-hub-surface-subtle)]">
                    Открыть документ
                    <ArrowUpRight className="size-4" aria-hidden />
                  </LinkRenderer>
                  <span className="text-xs font-normal text-[var(--legal-hub-content-inverse)]">Редакция от {document.updatedAt}</span>
                </div>
              </AccordionContent>
            </AccordionItem>
          ))}
