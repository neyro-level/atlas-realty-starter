...
export function LegalHubView({ documents, linkRenderer: LinkRenderer }: LegalHubViewProps) {
  return (
    <main className="min-h-[var(--viewport-content-below-header)] bg-[var(--legal-hub-surface-primary)] px-4 py-8 text-[var(--legal-hub-content-primary)] sm:px-6 md:py-14 lg:py-16">
      <Card className="mx-auto max-w-285 overflow-hidden rounded-lg border-[var(--legal-hub-border-primary)] bg-white px-5 py-7 shadow-[var(--legal-hub-shadow-primary)] sm:px-7 sm:py-9 md:px-10 md:py-11">
        ...
        <h1 className="text-section-prominent font-semibold leading-tight text-[var(--legal-hub-content-tertiary)] md:text-display-base">Правовая информация</h1>
        ...
        <Accordion type="multiple" className="mt-8 border-t border-[var(--legal-hub-border-secondary)] md:mt-10">
          {documents.map((document) => (
            <AccordionItem key={document.id} value={document.slug} className="group border-[var(--legal-hub-border-secondary)]">
              <AccordionTrigger
                className="min-h-18 py-5 text-body-compact font-medium leading-6 text-[var(--legal-hub-content-subtle)] hover:text-[var(--legal-hub-content-subtle)] md:min-h-20.5 md:py-6 md:text-base"
              ...
              <AccordionContent forceMount className="pb-7 pr-0 md:pb-8 md:pr-16">
                <p className="max-w-190 text-sm font-normal leading-6 text-[var(--legal-hub-content-strong)] md:text-body-compact">{document.description}</p>
