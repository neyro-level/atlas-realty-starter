    ...
    <main className="bg-[var(--legal-document-surface-primary)] text-[var(--legal-document-content-primary)]">
      <header className="border-b border-[var(--legal-document-border-primary)] bg-[var(--surface-card)]">
        <div className="mx-auto max-w-295 px-5 py-8 md:px-8 md:py-10">
          ...
          <h1 className="mt-5 max-w-225 text-heading-large font-extrabold leading-[1.12] text-[var(--text-primary)] md:text-display-medium">{document.title}</h1>
          <p className="mt-4 max-w-190 text-body-compact leading-6 text-[var(--legal-document-content-tertiary)] md:text-base md:leading-7">{document.description}</p>
        ...
        <Card className="overflow-hidden rounded-lg border-[var(--legal-document-border-primary)] bg-[var(--surface-card)] px-5 md:px-8">
          {document.sections.map((section, index) => (
            <section id={`section-${index + 1}`} key={section.title} className="scroll-mt-32 pt-7 last:pb-7 md:pt-9 md:last:pb-9">
              ...
              {section.paragraphs?.length ? <div className="mt-4 grid gap-3 text-body-compact leading-[1.65] text-[var(--legal-document-content-strong)] md:text-base">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div> : null}
              {section.items?.length ? <ul className="mt-4 grid gap-2.5 text-body-compact leading-[1.6] text-[var(--legal-document-content-strong)] md:text-base">{section.items.map((item) => <li key={item} className="grid grid-cols-[8px_minmax(0,1fr)] gap-3"><span className="mt-2.25 size-1.5 rounded-full bg-[var(--accent)]" aria-hidden /><span>{item}</span></li>)}</ul> : null}
      ...
      <section className="border-t border-[var(--legal-document-border-primary)] bg-[var(--surface-card)]">
        <div className="mx-auto flex max-w-295 flex-col gap-3 px-5 py-7 text-sm text-[var(--legal-document-content-hover)] md:flex-row md:items-center md:justify-between md:px-8">
          <p>{legalName}</p>
