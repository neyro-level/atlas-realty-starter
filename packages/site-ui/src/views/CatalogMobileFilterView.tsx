...
function RangeField({ from, to, onFrom, onTo, fromPlaceholder, toPlaceholder, inputMode = "numeric", numericOnly = true }: { from: string; to: string; onFrom: (value: string) => void; onTo: (value: string) => void; fromPlaceholder: string; toPlaceholder: string; inputMode?: "none" | "text" | "tel" | "url" | "email" | "numeric" | "decimal" | "search"; numericOnly?: boolean }) {
  ...
  return <div className="min-w-0"><div className="flex min-h-11 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-02)] px-3"><Input unstyled inputMode={inputMode} value={from} onChange={(event) => onFrom(normalize(event.target.value))} placeholder={fromPlaceholder} className="min-w-0 flex-1 bg-transparent text-[13px] font-medium tabular-nums text-[var(--text-primary)] outline-none placeholder:text-[var(--catalog-mobile-filter-content-01)]" /><span className="text-[12px] text-[var(--catalog-mobile-filter-content-01)]">–</span><Input unstyled inputMode={inputMode} value={to} onChange={(event) => onTo(normalize(event.target.value))} placeholder={toPlaceholder} className="min-w-0 flex-1 bg-transparent text-[13px] font-medium tabular-nums text-[var(--text-primary)] outline-none placeholder:text-[var(--catalog-mobile-filter-content-01)]" /></div></div>;
}
