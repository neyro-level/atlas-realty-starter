    ...
    return (
      <div className="mt-4 space-y-2.5 lg:hidden" data-new-building-mobile-filter>
        ...
        <Sheet open={filtersSheetOpen} onOpenChange={setFiltersSheetOpen}>
          <SheetContent side="bottom" className="inset-x-0 bottom-0 max-h-[85dvh] overflow-y-auto rounded-t-2xl border-0 bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[var(--catalog-mobile-filter-shadow-drawer)] lg:hidden" showClose>
            ...
            <div className="space-y-3">
              ...
              <label className="block text-[11px] font-semibold uppercase text-[var(--text-muted)]">
                Порядок
                <Select unstyled value={draft.sort} onChange={(event) => setDraft((current) => ({ ...current, sort: event.target.value }))} className="mt-1.5 min-h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3 text-[13px] font-medium normal-case text-[var(--text-primary)]">
...
function SearchField({ draft, setDraft }: { draft: MobileFilterDraftViewDto; setDraft: Dispatch<SetStateAction<MobileFilterDraftViewDto>> }) {
  return <label className="flex min-h-11 items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3"><Search className="size-4 shrink-0 text-[var(--text-muted)]" aria-hidden /><span className="sr-only">Поиск по новостройкам</span><Input unstyled value={draft.q} onChange={(event) => setDraft((current) => ({ ...current, q: event.target.value }))} placeholder="Название ЖК, район или застройщик" className="min-w-0 flex-1 bg-transparent text-[13px] font-medium outline-none placeholder:text-[var(--text-muted)]" /></label>;
}

function ApplyButton({ label, onClick }: { label: string; onClick: () => void }) {
  return <Button variant="plain" type="button" onClick={onClick} className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-[var(--surface-dark)] px-4 text-[13px] font-semibold text-white transition hover:bg-[var(--catalog-mobile-filter-surface-inverse)]">{label}</Button>;
}
...
function MobileFilterFields({ draft, setDraft, facets, typeSummary, sortLabel, sortOpen, setSortOpen, sortRef, advancedOpen, setAdvancedOpen, showAdvancedAlways, onOpenType, onToggleRoom, onClear }: {
  ...
  return <>
    <label className="flex min-h-11 items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3"><Search className="size-4 shrink-0 text-[var(--text-muted)]" aria-hidden /><span className="sr-only">Поиск по каталогу</span><Input unstyled value={draft.q} onChange={(event) => setDraft((current) => ({ ...current, q: event.target.value }))} placeholder="Поиск ЖК, улицы, района" className="min-w-0 flex-1 bg-transparent text-[13px] font-medium outline-none placeholder:text-[var(--text-muted)]" /></label>
    <Button variant="plain" type="button" onClick={onOpenType} className="flex min-h-11 w-full items-center justify-between gap-3 rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3 text-left text-[13px] font-medium text-[var(--text-primary)]"><span className="min-w-0 truncate">{typeSummary}</span><ChevronDown className="shrink-0 text-[var(--text-muted)]" aria-hidden /></Button>
    ...
    {advancedVisible ? <div className="space-y-2 rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-panel)] p-3">
...
function SmallSelect({ title, value, options, onChange }: { title: string; value: string; options: Array<{ value: string; label: string }>; onChange: (value: string) => void }) {
  return <label className="block text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--text-muted)]">{title}<Select unstyled value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3 text-[13px] font-medium normal-case tracking-[0] text-[var(--text-primary)]"><option value="">Любой</option>{options.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></label>;
}
...
function RangeField({ from, to, onFrom, onTo, fromPlaceholder, toPlaceholder, inputMode = "numeric", numericOnly = true }: { from: string; to: string; onFrom: (value: string) => void; onTo: (value: string) => void; fromPlaceholder: string; toPlaceholder: string; inputMode?: "none" | "text" | "tel" | "url" | "email" | "numeric" | "decimal" | "search"; numericOnly?: boolean }) {
  ...
  return <div className="min-w-0"><div className="flex min-h-11 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--catalog-mobile-filter-surface-control)] px-3"><Input unstyled inputMode={inputMode} value={from} onChange={(event) => onFrom(normalize(event.target.value))} placeholder={fromPlaceholder} className="min-w-0 flex-1 bg-transparent text-[13px] font-medium tabular-nums text-[var(--text-primary)] outline-none placeholder:text-[var(--catalog-mobile-filter-content-muted)]" /><span className="text-[12px] text-[var(--catalog-mobile-filter-content-muted)]">–</span><Input unstyled inputMode={inputMode} value={to} onChange={(event) => onTo(normalize(event.target.value))} placeholder={toPlaceholder} className="min-w-0 flex-1 bg-transparent text-[13px] font-medium tabular-nums text-[var(--text-primary)] outline-none placeholder:text-[var(--catalog-mobile-filter-content-muted)]" /></div></div>;
}
