...
export function NewBuildingQuickSelectionsView({ items, linkRenderer: Link }: { items: readonly NewBuildingQuickSelectionItem[]; linkRenderer: SiteLinkRenderer }) {
  return (
    <section className="mb-5 lg:hidden" aria-labelledby="new-building-quick-selections-title" data-new-building-quick-selections>
      <h2 id="new-building-quick-selections-title" className="text-card-large font-extrabold leading-[1.08] md:text-section-small">Выберите, что для вас важно</h2>
    ...
    <section className="grid gap-3 pt-7 md:gap-4 md:pt-9 lg:hidden" aria-label={`Цены, планировки и ипотека в ${detail.name}`} data-new-building-mobile-commercial>
      <Card className="overflow-hidden rounded-lg bg-[var(--surface-card)] shadow-[var(--new-building-section-shadow-raised)]">
        ...
        <CardContent className="p-5 pt-0 md:p-6 md:pt-0">
          ...
          <div className="mt-5 rounded-lg bg-[var(--surface-card-soft)] p-4 md:p-5">
            <h2 className="text-heading-small font-extrabold leading-tight text-[var(--text-primary)] md:text-heading-medium">Получите актуальные цены и планировки</h2>
        ...
        <CardFooter className="flex-col items-stretch gap-3 p-5 pt-0 md:p-6 md:pt-0">
          {primaryAction}
          <p className="flex items-start gap-2 text-xs font-medium leading-5 text-[var(--text-secondary)]"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-[var(--status-success)]" aria-hidden />{serviceNote}</p>
        </CardFooter>
      </Card>
      ...
      <Card className="overflow-hidden rounded-lg border-[var(--surface-dark)] bg-[var(--surface-dark)] text-white shadow-[var(--new-building-section-shadow-raised)] md:grid md:grid-cols-[minmax(0,1fr)_240px] md:items-center">
        <CardHeader className="p-5 md:p-6">
          ...
          <CardTitle className="mt-4 text-heading-compact font-extrabold leading-[1.14] text-white md:text-section-base">Поможем разобраться с ипотекой</CardTitle>
          ...
          <p className="pt-4 text-sm font-bold leading-5 text-white">{mortgageNote}</p>
        </CardHeader>
  ...
  return (
    <section className="bg-[var(--surface-card)] pt-6 text-[var(--text-primary)] md:pt-7 lg:hidden" aria-labelledby="mobile-why-agency-title">
      <Card className="rounded-lg bg-[var(--surface-card-soft)] p-5 shadow-[var(--new-building-section-shadow-raised)] md:p-6">
        ...
        <h2 id="mobile-why-agency-title" className="mt-4 text-section-small font-extrabold leading-[1.12] md:text-section-prominent">Почему выгодно покупать через {brand}</h2>
      ...
      <div className="mx-auto max-w-site-frame px-5">
        <Card className="grid overflow-hidden rounded-lg bg-[var(--surface-card-soft)] shadow-[var(--new-building-section-shadow-raised)] lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
          <div className="flex flex-col px-5 py-10 sm:px-10 sm:py-12 lg:px-14 lg:py-16">
            ...
            <h2 id="new-building-why-agency-title" className="mt-7 max-w-180 text-heading-large font-extrabold leading-[1.08] sm:text-display-medium lg:text-display-extra-large">Почему новостройку выбирают через <span className="text-[var(--accent)]">{brand}</span></h2>
            <p className="mt-5 max-w-162.5 text-body-compact leading-7 text-[var(--text-secondary)] sm:text-base">Смотрим на рынок целиком и остаёмся на стороне покупателя от первого сравнения до сделки.</p>
...
export function NewBuildingPurchaseProcessView({ steps }: { steps: readonly { title: string; description: string }[] }) {
  return (
    <section className="bg-[var(--surface-card-soft)] py-12 sm:py-16 lg:py-22" aria-labelledby="new-building-process-title">
      <div className="mx-auto max-w-site-frame px-5">
        <h2 id="new-building-process-title" className="max-w-220 text-section-small font-semibold leading-[1.1] sm:text-heading-large">Как проходит подбор и покупка: от первого обращения до сделки</h2>
        <ol className="relative mt-12 grid max-w-225 gap-9 before:absolute before:bottom-4 before:left-[23px] before:top-4 before:w-px before:bg-[var(--border)]">
          {steps.map((step, index) => <li key={step.title} className="relative grid grid-cols-[48px_minmax(0,1fr)] gap-5"><span className="relative z-10 flex h-12.5 items-center bg-[var(--surface-card-soft)] pr-2 text-heading-extra-large font-light leading-none text-[var(--accent)]">{String(index + 1).padStart(2, "0")}</span><div className="pt-1"><h3 className="text-lg font-semibold leading-snug">{step.title}</h3><p className="mt-3 max-w-180 text-sm leading-6 text-[var(--text-secondary)]">{step.description}</p></div></li>)}
        </ol>
      </div>
    </section>
  );
