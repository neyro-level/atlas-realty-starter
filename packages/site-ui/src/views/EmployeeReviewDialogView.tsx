...
export function EmployeeReviewDialogView({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100vh-32px)] overflow-y-auto rounded-lg border border-[var(--employee-review-dialog-border-01)] bg-white p-6 shadow-[var(--employee-review-dialog-shadow-01)] md:p-8">
        ...
        {result?.ok ? (
          <div className="mt-7 rounded-lg bg-[var(--employee-review-dialog-surface-01)] p-5 text-[var(--employee-review-dialog-content-01)]" role="status" aria-live="polite">
          ...
          <form onSubmit={onSubmit} data-analytics-form-type="employee_review" className="mt-7 grid gap-4">
            ...
            <label className="grid gap-2 text-sm font-medium text-[var(--text-primary)]">
              ...
              <Input unstyled
                ...
                className="min-h-12 rounded-lg border border-[var(--employee-review-dialog-border-02)] px-4 text-base font-medium tabular-nums outline-none transition placeholder:text-[var(--employee-review-dialog-content-03)] focus:border-[var(--accent)]"
              />
              {result?.fieldErrors?.authorPhone ? <span className="text-xs text-[var(--accent)]">{result.fieldErrors.authorPhone}</span> : null}
              <span className="flex items-center gap-2 text-[11px] font-normal leading-4 text-[var(--employee-review-dialog-content-04)]">
            ...
            <label className="grid gap-2 text-sm font-medium text-[var(--text-primary)]">
              Отзыв
              <Textarea unstyled name="text" rows={5} className="resize-none rounded-lg border border-[var(--employee-review-dialog-border-02)] px-4 py-3 text-base font-normal outline-none transition focus:border-[var(--accent)]" />
...
function Field({ label, name, type = "text", autoComplete, error }: { label: string; name: string; type?: string; autoComplete?: string; error?: string }) {
  return (
    <label className="grid gap-2 text-sm font-medium text-[var(--text-primary)]">
      {label}
      <Input unstyled name={name} type={type} autoComplete={autoComplete} className="min-h-12 rounded-lg border border-[var(--employee-review-dialog-border-02)] px-4 text-base font-normal outline-none transition focus:border-[var(--accent)]" />
