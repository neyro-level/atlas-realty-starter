        ...
        <section className="bg-white"><div className="mx-auto max-w-site-frame px-5 pb-3 pt-6 lg:py-8"><div className="mb-4">{breadcrumbs}</div>{page.usesCatalogHero ? catalogHero : (
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.18fr)_minmax(360px,0.82fr)]">
            <div className="relative min-h-115 overflow-hidden rounded-xl bg-[var(--surface-dark)] text-white shadow-[var(--corporate-landing-shadow-primary)]">
              ...
              <div className="relative z-10 flex h-full flex-col justify-between gap-8 p-6 md:p-8 lg:p-10"><div className="max-w-3xl"><p className="text-caption font-extrabold uppercase tracking-[0.18em] text-white/74">{page.eyebrow}</p><h1 className="mt-5 text-[clamp(2.2rem,4.4vw,4.2rem)] font-extrabold leading-[0.94] text-white">{page.heroTitle}</h1><p className="mt-5 max-w-2xl text-base leading-7 text-white/78">{page.heroDescription}</p></div><div className="flex flex-col gap-4">{primaryAction}<p className="max-w-2xl text-sm leading-6 text-white/66">{page.microtext}</p></div></div>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">{page.heroCards.map((card, index) => { const Icon = ICONS[card.icon ?? DEFAULT_ORDER[index % DEFAULT_ORDER.length]!]; return <Card key={card.title} className="rounded-lg border-[var(--border)] bg-white p-5 shadow-none transition hover:shadow-[var(--corporate-landing-shadow-secondary)]"><div className="flex size-11 items-center justify-center rounded-lg bg-[var(--accent-soft)] text-caption font-extrabold uppercase tracking-[0.16em] text-[var(--accent)]"><Icon className="size-5" aria-hidden /></div><h2 className="mt-5 text-body-emphasis font-extrabold leading-tight text-[var(--text-primary)]">{card.title}</h2><p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{card.text}</p></Card>; })}</div>
          </div>
        )}</div></section>
      )}
      {bodyBeforeRelated}
