      ...
      {page.hasSecondaryCatalogIntro ? null : (
        <section className="bg-white"><div className="mx-auto max-w-site-frame px-5 pb-3 pt-6 lg:py-8"><div className="mb-4">{breadcrumbs}</div>{page.usesCatalogHero ? catalogHero : (
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.18fr)_minmax(360px,0.82fr)]">
            <div className="relative min-h-[460px] overflow-hidden rounded-xl bg-[var(--surface-dark)] text-white shadow-[var(--corporate-landing-shadow-01)]">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">{page.heroCards.map((card, index) => { const Icon = ICONS[card.icon ?? DEFAULT_ORDER[index % DEFAULT_ORDER.length]!]; return <Card key={card.title} className="rounded-lg border-[var(--border)] bg-white p-5 shadow-none transition hover:shadow-[var(--corporate-landing-shadow-02)]"><div className="flex size-11 items-center justify-center rounded-lg bg-[var(--accent-soft)] text-[11px] font-extrabold uppercase tracking-[0.16em] text-[var(--accent)]"><Icon className="size-5" aria-hidden /></div><h2 className="mt-5 text-[1.05rem] font-extrabold leading-tight text-[var(--text-primary)]">{card.title}</h2><p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">{card.text}</p></Card>; })}</div>
          </div>
        )}</div></section>
      )}
      {bodyBeforeRelated}
