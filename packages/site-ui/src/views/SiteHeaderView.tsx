...
export function SiteHeaderView(props: SiteHeaderViewProps) {
  const {
    headerRef, mobileButtonRef, pathname, contacts, desktopNav, cityOptions, brand, brandLabel, mobileMenu, compareAction, favoritesAction,
  ...
  const headerClass = compactSticky
    ? "fixed left-3 right-3 top-3 z-50 mx-auto h-13 max-w-site-frame-floating rounded-2xl border border-[var(--border)] bg-white/96 shadow-[var(--site-header-shadow-primary)] backdrop-blur-xl transition-all duration-300 lg:h-26.5"
    : docked
      ? "fixed left-3 right-3 top-3 z-50 mx-auto h-17 max-w-site-frame-floating rounded-[18px] border border-[var(--border)] bg-white/96 shadow-[var(--site-header-shadow-secondary)] backdrop-blur-xl transition-all duration-300 lg:h-26.5"
      : "fixed left-0 right-0 top-0 z-50 h-17 border-b border-[var(--border)] bg-white/92 shadow-[var(--site-header-shadow-tertiary)] backdrop-blur-xl transition-all duration-300 lg:h-26.5";
          ...
          {compactSticky ? (
            <div className="flex h-full items-center gap-3 lg:hidden">
              {sessionCollectionPage ? (
                <Button asChild variant="secondary" size="sm" className="min-h-9 flex-1 gap-1.5 rounded-lg bg-[var(--surface-muted)] px-3 text-label text-[var(--text-secondary)] hover:bg-[var(--border)] hover:text-[var(--accent)]">
              ...
              <Button type="button" variant="secondary" size="sm" onClick={onOpenFilters} className="min-h-9 flex-1 gap-1.5 rounded-lg bg-[var(--surface-muted)] px-3 text-label text-[var(--text-secondary)] hover:bg-[var(--border)]">
          ...
          <div className={compactSticky ? "hidden h-full lg:block" : "h-full"}>
            <div className="flex h-full items-center gap-3 lg:h-14 xl:gap-4">
              ...
              <div className="ml-auto flex shrink-0 items-center justify-end gap-2">
                ...
                <Button ref={mobileButtonRef} type="button" variant="outline" size="icon" aria-label={mobileOpen ? "Закрыть меню" : "Открыть меню"} aria-expanded={mobileOpen} aria-controls="site-mobile-menu" className="bg-white text-[var(--foreground)] lg:hidden" onClick={onToggleMobile}>
