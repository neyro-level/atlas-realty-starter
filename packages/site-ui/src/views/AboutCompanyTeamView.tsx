...
export function AboutCompanyTeamView({ photos, imageRenderer: ImageRenderer }: { photos: readonly AboutCompanyTeamPhoto[]; imageRenderer: SiteImageRenderer }) {
  ...
  return (
    <section className="bg-[var(--palette-f9fafb)] py-12 sm:py-16 lg:py-[72px]" aria-labelledby="about-team-title">
      <div className="mx-auto max-w-site-frame px-5">
        ...
        <p className="mt-4 max-w-[820px] text-[16px] leading-7 text-[var(--palette-5e5a5f)] lg:max-w-none lg:whitespace-nowrap">Агент ведёт клиента на каждом этапе, а юрист и ипотечный брокер подключаются там, где это необходимо.</p>
